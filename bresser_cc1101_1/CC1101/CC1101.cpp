...
printf("try to find the CC1101 chip \n");
  ...
  while((i < 3) && !flagFound) {
    ...
    if((version == RADIOLIB_CC1101_VERSION_CURRENT) || (version == RADIOLIB_CC1101_VERSION_LEGACY) || (version == RADIOLIB_CC1101_VERSION_CLONE)) {
      ...
      printf("CC1101 not found! (%d of 10 tries) RADIOLIB_CC1101_REG_VERSION == 0x%04X, expected 0x0004/0x0014\n", i + 1, version);
...
state = variablePacketLengthMode();
  ...
  state = setEncoding(RADIOLIB_ENCODING_NRZ); // RADIOLIB_ENCODING_MANCHESTER   RADIOLIB_ENCODING_NRZ
...
/*  
printf("go to rx mode\n");

int16_t ss=SPIgetRegValue(RADIOLIB_CC1101_REG_MARCSTATE, 4, 0);
printf("reg 1 is %d\n", ss);
while(ss != RADIOLIB_CC1101_MARC_STATE_RX) {

  
    usleep(10000); 
ss=SPIgetRegValue(RADIOLIB_CC1101_REG_MARCSTATE, 4, 0);
printf("reg x is %d\n", ss);
}
*/
...
//readData(data, len);
  ...
  //uint8_t rfifo  = SPIreadRegister(RADIOLIB_CC1101_REG_FIFO);



  uint8_t rlen  = SPIreadRegister(0xFB);
   ...
   // read packet data
  SPIreadRegisterBurst(RADIOLIB_CC1101_REG_FIFO  , len, data);
standby();
 ...
 // return(123);
  ...
  while(SPIgetRegValue(RADIOLIB_CC1101_REG_MARCSTATE, 4, 0) != RADIOLIB_CC1101_MARC_STATE_IDLE) {
   ...
   printf(".");
    ...
    if(this->mod->hal->micros() - start > 100000) {
...
int16_t CC1101::startReceive() {
 ...
 //Victor RADIOLIB_CC1101_GDOX_HIGH_Z
 state = SPIsetRegValue(RADIOLIB_CC1101_REG_IOCFG0, RADIOLIB_CC1101_GDOX_HIGH_Z, 6, 0);
...
//uint8_t rfifo=SPIreadRegister(RADIOLIB_CC1101_REG_FIFO);;
//uint8_t rlen=SPIreadRegister(RADIOLIB_CC1101_REG_PKTLEN);
...
uint8_t CC1101::SPIreadRegister2(uint8_t reg) {
  // status registers require special command
  if((reg > RADIOLIB_CC1101_REG_TEST0) && (reg < RADIOLIB_CC1101_REG_PATABLE)) {
    reg |= RADIOLIB_CC1101_CMD_ACCESS_STATUS_REG;
  }
